...
export function LandingPage() {
  ...
  // Subtle depth: each parallax shape drifts a small, distinct amount as the hero scrolls away.
  // Ranges stay within ~20-45px so it reads as depth, not floating objects.
  const yellowRectY = useTransform(heroScrollProgress, [0, 1], [0, shouldReduceMotion ? 0 : 60]);
  const navyBlockY = useTransform(heroScrollProgress, [0, 1], [0, shouldReduceMotion ? 0 : 45]);
    ...
    <ReactLenis root options={{ anchors: true }}>
      <div className="min-h-screen overflow-x-clip bg-white text-[#454650]">
        ...
        <main>
          ...
          <section
            ref={heroRef}
            className="relative flex flex-col items-center overflow-hidden px-10 pt-32 pb-24"
          >
            ...
            <motion.div
              style={{ y: navyBlockY }}
              className="hidden sm:block absolute -right-20 bottom-20 size-64 rounded-l-full bg-[#001456] opacity-90"
            />

            <motion.div
              style={{ y: yellowRectY }}
              className="absolute bottom-20 left-12 h-32 w-16 bg-[#fdd34d] opacity-90"
            />
